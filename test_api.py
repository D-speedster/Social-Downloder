...
def test_tiktok_api():
    try:
        ...
        print(f"TikTok API Status Code: {res.status}")
        print(f"TikTok API Response: {data.decode('utf-8')[:1000]}")
        
        if res.status == 200:
            response_data = json.loads(data.decode('utf-8'))
            print(f"TikTok API Response Data: {response_data}")
        else:
            print(f"TikTok API Error: {res.status}")
