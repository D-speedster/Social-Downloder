...
async def test_direct_http_proxy():
    """تست مستقیم با HTTP proxy پورت 10808"""
    
    test_url = "https://www.youtube.com/watch?v=dQw4w9WgXcQ"
    
    # تنظیمات مستقیم برای HTTP proxy
    opts = {
...
async def test_with_rotation_http():
    """تست با سیستم rotation ما - فقط HTTP proxy"""
    
    # تنظیم متغیرهای محیطی برای استفاده فقط از HTTP proxy
    os.environ['YOUTUBE_ENABLE_SOCKS'] = '0'  # غیرفعال کردن SOCKS5
    if 'YOUTUBE_SINGLE_PROXY' in os.environ:
        del os.environ['YOUTUBE_SINGLE_PROXY']  # حذف پروکسی واحد
    ...
    try:
        ...
        if result and isinstance(result, dict):
            print("✅ SUCCESS! Rotation system with HTTP proxy completed successfully")
            print(f"📹 Title: {result.get('title', 'N/A')}")
            print(f"⏱️  Duration: {result.get('duration', 'N/A')} seconds")
            print(f"👀 Views: {result.get('view_count', 'N/A')}")
            print(f"📺 Channel: {result.get('uploader', 'N/A')}")
...
async def test_single_http_proxy():
    """تست با تنظیم پروکسی واحد HTTP"""
    
    # تنظیم پروکسی واحد HTTP
    ...
    print(f"📺 URL: {test_url}")
    print(f"🔧 Single proxy: http://127.0.0.1:10808")
    print("=" * 60)
...
if __name__ == "__main__":
    print("🚀 HTTP Proxy (10808) Comprehensive Test")
    print("=" * 80)
    ...
    async def run_all_tests():
        ...
        # تست 1: مستقیم
        print("TEST 1: Direct YoutubeDL with HTTP proxy")
        print("=" * 80)
        results['direct'] = await test_direct_http_proxy()
        
        # تست 2: با سیستم rotation (فقط HTTP)
        print("\n" + "=" * 80)
        print("TEST 2: Rotation system with HTTP proxy only")
        print("=" * 80)
        results['rotation'] = await test_with_rotation_http()
